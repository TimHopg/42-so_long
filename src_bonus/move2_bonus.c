...
/*   Updated: 2024/07/16 21:20:41 by thopgood         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	img_to_img(vars->xpm[BG], vars->xpm[img], ((vars->map->p_x + x) * TSZ) + 8,
		((vars->map->p_y + y) * TSZ) + 8);
	img_to_img(vars->xpm[BG], vars->xpm[FIELD], vars->map->p_x * TSZ,
		vars->map->p_y * TSZ);
	if (vars->map->p_x == vars->map->exit_x
		&& vars->map->p_y == vars->map->exit_y)
		img_to_img(vars->xpm[BG], vars->xpm[CHESTC], (vars->map->p_x * TSZ) + 8,
			(vars->map->p_y * TSZ) + 8);
...
void	turn(t_vars *vars, int img)
{
	if (vars->map->map[vars->map->p_y][vars->map->p_x] == 'E')
		...
		img_to_img(vars->xpm[BG], vars->xpm[FIELD], vars->map->p_x * TSZ,
			vars->map->p_y * TSZ);
	img_to_img(vars->xpm[BG], vars->xpm[img], ((vars->map->p_x) * TSZ) + 8,
		((vars->map->p_y) * TSZ) + 8);
	mlx_put_image_to_window(vars->mlx, vars->win, vars->xpm[BG].img_ptr, 0, 0);
}
